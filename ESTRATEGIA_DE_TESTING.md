...
| 5 | **Frontend con catálogo hardcodeado (`menu.html`)** | `menu.html` contiene 40 productos hardcodeados con `data-id` 1–40, nombres e imágenes embebidas. `Build.js` arma `window.pcBuild` a partir de esos `dataset.id` y `Presupuestar.js` envía `productoId` real al backend. El backend ya expone `GET /productos` y `GET /productos/{idCategoria}`, pero el frontend no los consume. **Riesgo:** los IDs hardcodeados dependen del orden de inserción del `DataSeeder` (`GenerationType.IDENTITY`); si el seeder cambia, el frontend rompe silenciosamente. Además `ProductoResponseDTO` no expone `categoriaId` ni `imagenUrl`, por lo que el render dinámico requeriría extender el DTO. **Decisión tomada (2026-08-10):** se acepta como deuda para el MVP (proyecto de portafolio). El `DataSeeder` es la fuente de verdad de los productos. Si se retoma, el plan es: (1) extender `ProductoResponseDTO` con `categoriaId`/`imagenUrl`, (2) render dinámico en `menu.html` desde la API, (3) adaptar `Build.js` para recibir el producto completo. | ✅ Aceptada — documentada |
| 6 | **Test de integración de `PedidoService` (flujo crear→confirmar→stock)** | El flujo crítico de pedidos (crear → confirmar → descuento de stock) solo está cubierto por tests unitarios con mocks. Un test de integración con `@SpringBootTest` + H2 en modo MySQL + `@Transactional` verificaría el comportamiento real de JPA/Hibernate (flush, transacciones, stock). **Decisión tomada (2026-08-10):** se difiere como deuda para el MVP. Los 130 tests actuales (FASE 1+2+3) cubren la lógica de negocio y el contrato HTTP; el riesgo residual es bajo para un proyecto de portafolio. | ✅ Aceptada — diferida |
| 7 | **`System.out.println(pedidosCreateDTO)` en `PedidosController.editarPedido`** | Log de debug a consola que imprime el DTO recibido. No genera impacto negativo en producción (solo ruido en consola), pero no es una práctica profesional: en proyectos reales se usa un logger (`SLF4J`/`Logback`) con niveles. Se agregó originalmente para debuggear un bug. **Decisión tomada (2026-08-10):** se acepta como deuda; no se elimina para no tocar código que funciona. Si se retoma, reemplazar por `log.info(...)` con `@Slf4j`. | ✅ Aceptada — documentada |
